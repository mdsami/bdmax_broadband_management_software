...
/*------------ End Conect DB----------------*/
$id=$_GET['id'];
$sql="SELECT * FROM `tblbilling` where sl_no='$id'"; 					// Sql Query to Select all Data from DB
...
?>
	...
	<form name="form1" method="post" action="update_action.php"> 
		<p>
		Receiver Name:
				<input type="text" name="receivername" value="<?php echo $rows['Receiver_Name']; ?>">
	  ...
	  <p>Date 
	     <input type="text" name="date" value="<?php echo $rows['Date']; ?>">
		 
		 
		 <p>
		Month:
				<input type="text" name="month" value="<?php echo $rows['Month']; ?>">
		...
		Bill:
		<input type="text" name="bill" value="<?php echo $rows['Bill']; ?>">
		 

		</p>
		Due:
		<input type="text" name="due" value="<?php echo $rows['Due']; ?>">
		 

		</p>
		Total:
		<input type="text" name="total" value="<?php echo $rows['Total']; ?>">
		
		</p>
		Status:
		<input type="text" name="status" value="<?php echo $rows['Status']; ?>">
